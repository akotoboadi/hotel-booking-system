...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.email === 'guest@akstay.com' && form.password === 'guest123') {
        login(
          { name: 'Kofi Asante', email: form.email, id: 1, avatar: null },
          'mock-guest-token'
        )
        navigate('/')
      } else {
        setError('Invalid email or password. Try guest@akstay.com / guest123')
      }
    } finally {
      setLoading(false)
    }
  ...
  const handleGmailLogin = async () => {
    ...
    setError('')
    try {
      // Simulated Gmail OAuth — replace with real Google OAuth when backend is ready
  ...
  return (
    <div className="auth-page">
      <div className="auth-card">
        ...
        >
          {gmailLoading ? (
            <span className="gmail-btn__spinner" />
          ) : (
            <GoogleIcon />
          )}
          {gmailLoading ? 'Signing in with Google...' : 'Continue with Google'}
        </button>
      ...
      <div className="auth-visual">
        <img
          src="https://images.unsplash.com/photo-1455587734955-081b22074882?w=800"
          alt="luxury hotel"
        />
...
// Inline Google SVG icon — no extra package needed
function GoogleIcon() {
  return (
